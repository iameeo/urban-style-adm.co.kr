...
public class MultipartUtilityGD {
	private static final Logger log = getLogger(MultipartUtilityGD.class.getName());
	...
	public MultipartUtilityGD(final URL url, String gdCookie) throws IOException, NoSuchAlgorithmException, KeyManagementException {
		...
		if(this.url.toString().contains("https://")){
			...
			connection.setRequestProperty("cookie", gdCookie);
			connection.setRequestProperty("Cookie", gdCookie.replace("{", "").replace("}", ""));
	...
	public void addFilePart(final String fieldName, final File uploadFile) throws IOException {
		...
		writer.append("--").append(boundary).append(CRLF).append("Content-Disposition: form-data; name=\"")
				...
				.append(fieldName).append("\"; filename=\"").append("").append("\"").append(CRLF)
				.append(CRLF);
